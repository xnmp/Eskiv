...
def Poset(pos, radius):
	range0 = range(pos[0] - radius, pos[0] + radius)
	range1 = range(pos[1] - radius, pos[1] + radius)
	return {p for p in zip(range0, range1) if Dist(p, pos) < radius}
...
class FuzzyBall(Eskiv):
	...
	def __init__(self, pos = None, poset = set(), balls = set(), velo = None, color = None):
		...
		if balls != set():
			...
			if velo == None:
				self.balls = {Ball(pos, v) for pos in poset for v in [(-1,0),(1,0),(0,-1),(0,1)]}
			else:
				self.balls = {Ball(pos, velo) for pos in poset}
	...
	def diam(self):
		...
		return max(self.limit[0][1] - self.limit[0][0], self.limit[1][1] - self.limit[1][0])
	...
	def center(self):
		...
		for i in range(2):
			self.limit[i][0] = min(self.poset, key = lambda x: x[i][0])
			self.limit[i][1] = max(self.poset, key = lambda x: x[i][1])
		return ((self.limit[0][1] + self.limit[0][0]) / 2, (self.limit[1][1] - self.limit[1][0]) / 2)
...
def EvolveCone(poslist, tau = dt, speed = 0.1):
	#TODO: add all pixels in between
	dist = speed * dt / 2
	range0 = range(pos - dist, pos + dist)
	poslistout = {(x, y) for x in range0 for y in range0 if (0 <= x <= s_size[0] and 0 <= y <= s_size[1])}
...
class Ball(Eskiv):
	...
	def Poset(self):
		range0 = range(self.pos - self.radius / 2, self.pos + self.radius / 2)
		return {(x,y) for x in range0 for y in range0 if Dist((x,y), self.pos, 2) <= self.radius}
...
class State(object):
	...
	def Navigate(self, cpos):
		...
		cposlist = [self.cpos]
		return Intersect(newstate.Poset(), EvolveCone(cposlist, speed = self.circle.speed))
