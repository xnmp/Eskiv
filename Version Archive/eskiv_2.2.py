...
class FuzzyBall(Eskiv):
	...
	def __init__(self, pos = None, poset = set(), velo = None, color = None):
		self.poset = poset
		self.velo = velo
		#this is the range of the ball. Of the form (min, max)
		self.limits = [[0, x_size], [0, y_size]]
		if pos != None and color != None:
			self.pos_er = self.radius * (2 - self.color / 255)
			range0 = range(pos[0] - self.pos_er / 2, pos[0] + self.pos_er / 2)
			range1 = range(pos[1] - self.pos_er / 2, pos[1] + self.pos_er / 2)
			self.poset = {(x, y) for x in range0 for y in range1}
	...
	def Diam(self):
		...
		self.center = ((self.limit[0][1] + self.limit[0][0]) / 2, (self.limit[1][1] - self.limit[1][0]) / 2)
	...
	def Evolved(self, tau = dt):
		...
		tempFuzz = {EvolvePos(self.pos, self.velo) for pos in self.poset}
		self.poset = {x[0] for x in tempFuzz}
		# the velocity only flips when all members of the poset have flipped
		self.velo = Mode({x[1] for x in tempFuzz}) # broken right now
...
class Ball(FuzzyBall):
	...
	def Evolved(self, tau = dt):
		self.pos, self.velo  = EvolvePos(self.pos, self.velo)
		...
		for x,y in zip(range(x_size), range(y_size)):
			...
			if color[0] == 0 and color[2] > 0: 
				...
				if expected = False
					...
					if expected = False:
						...
						#add four fuzzy balls
						for v in [(-1,0),(1,0),(0,-1),(0,1)]:
							self.fballs.add(FuzzyBall(pos = (x,y), velo = v, color = color[2]))
						self.newballs -= 1
			...
			# 2. if the pixel is passable, update fballs
				...
				for ball in self.fballs:
					if (x,y) in ball.poset:
						#update the fball
						ball.poset.remove((x,y))
						#fball was fake
						if ball.poset == set():
							self.fballs.remove(ball)
						#fball reaches ball status
						elif ball.Diam() <= ball.radius + ball.tolerance:
							self.balls.add(Ball(pos = ball.center, velo = ball.velo))
							self.fballs.remove(ball)
