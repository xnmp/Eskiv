...
class Ball(Eskiv):
	speed = 0.1
	radius = 10
	#the pos is the best guess of the center, the bluest color
	#it can actually have up to four values
	def __init__(self, pos = None, velo = allvelos, pos_er = 0, color = 0, bid = ball_id):
		self.pos = pos
		self.pos_er = pos_er
		self.velo = velo
		self.color = color
	...
	def Update(self, dist, color):
		r1 = self.pos_er
		r2 = self.ColorToDist(color)
		self.pos = 

	def getError(self):
		return self.radius * (255 - self.color) / 255
...
class State(object):
	...
	def __init__(self, balls = [], bballs = [], circle = Circle(), square = Square(), newballs = 0):
		self.balls = balls
		self.bballs = bballs
		self.circle = circle
		self.square = square
	...
	def FirstUnknownBall(self):
		#the first unassigned ball
	...
	def Update(self, pixels):
		...
		for x,y in zip(range(x_size), range(y_size)):
			...
			# the pixel is either
			#	- passable and in the error radius of a ball -> update the ball
