...
top_bound, bottom_bound, left_bound, right_bound = 239, 823, 1196, 1867
...
def findBall(pixels, excl_poset = set(), tolerance = 20):
	...
	for y in range(0, len(pixels), 8):
		for x in range(0, len(pixels[0]), 8):
			...
			color = pixels[y][x]
			# found a pixel
			if color[0] == 0 and color[2]:
				# now search for the bluest pixel around it
				newsearch = Poset((x,y), Ball.radius, 1) - excl_poset
				maxblue = color[2]
				for (x,y) in newsearch:
					color = pixels[y][x]
					if maxblue < color[2]:
						maxblue = color[2]
						res_pos = (x,y)
				#print 'found ball with color ', maxblue, 'at', res_pos
				return res_pos, maxblue
	return None
...
class Ball(Eskiv):
	speed = 282.44 #in pixels per second
	radius = 5
	tolerance = 2
	...
	def Update(self, pixels):
		...
		# for pos in self.poset: #should check entire poset, but that is faulty
		pos = self.pos
		color = pixels[pos[0]][pos[1]]
		# print color
		if color[1] > 0:
			# if an empty pos infringes on a ball, remove it from fuzzy set
			pos2 = findBall(getPixels())
			print '~LOOP '+ str(loopnum) + ' removed ball with position', pos, 'and color', color
			print 'real pos:                        ', pos2
			return True
		return False
...
class FuzzySet(object):
	...
	def __init__(self, pos, color = None, last_time = 0):
	...
	def Reverse(self):
		...
		return FuzzySet({ball.Reverse() for ball in self.balls})
...
class State(object):
	...
	def Update(self, pixels):
		...
		if self.newballs >= 1:
			...
			newballpos, newballcolor = findBall(pixels, excl_poset = self.poset)
			if newballpos != None:
				tr('found new pixel')
				# TODO: add the color
				self.fsets.add(FuzzySet(pos = newballpos, color = newballcolor, last_time = time.time()))
				self.newballs -= 1
	...
	def Reverse(self):
		...
		return State(balls = {ball.Reverse() for ball in self.balls}, 
					fsets = {fsets.Reverse() for fset in self.fsets})
	...
	def getPath(self):
		...
		while cposets_b != []:
			...
			for velo in allvelos:
				cposet = self.circle.Evolved(velo).poset
...
while True:
	...
	for ball in state.balls:
		...
		# print 'expected ball pos: ', ball.pos, 'real ball pos: ', ballpos
		# guimove(ballpos)

		# measure the velocity
		# if loopnum >= 5:
		# 	vel = (lastballpos - ballpos[1])
		# 	totdist += np.absolute(vel)
		# 	print totdist / float(loopnum - 4)
