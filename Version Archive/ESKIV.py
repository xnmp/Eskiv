...
class Ball(Eskiv):
	speed = 0.1
	radius = 10
	#the pos is the best guess of the center 
	def __init__(self, pos = None, velo = allvelos, pos_er = 0, lastcolor = (0,0,255))
		self.pos = pos
		self.pos_er = pos_er
		self.velo = velo
...
class Circle(Eskiv):
	speed = 0.12
	radius = 15
	def __init__(self, pos = (0,0), pos_er = 0)
		self.pos = pos
		self.pos_er = pos_er
...
class State(object):
	...
	def __init__(self, balls = [], circle = Circle(), square = Square()):
		self.balls = balls
		self.circle = circle
		self.square = square
	...
	def Pixels(self):
		#takes state to pixels (not raw pixels)
		#helper function for Difference
		#true for passable, false for unpassable
		#pix = numpy.full((x_size, y_size), True, dtype=bool)
		pix = [[True] * x_size] * y_size
		for ballpos in flatten(self.pos()):
			pix[*ballpos] = False
		return pix
		#--TODO: make a radius around each of these points

	def Difference(self, pixels):
		#helper function to Update
		#gives the pixels that are filled in but shouldn't be
		#only look at the expected ones
		unexpected = []
		for ball in self.balls:
			ran = range(ball.pos - ball.pos_er, ball.pos + ball.pos_er)
			for x,y in zip(ran, ran):
				if x <= x_size and y <= y_size and pixels[x][y] == False:
					unexpected.append((x,y))
		return unexpected

	def Update(self, pixels, numballs = 0):
		#pixels to state
		...
		diff = newstate.Difference(pixels)

		#the new balls
		for i in range(numballs):
			pass

		#try to get rid of tuples
		for ballpos in newstate.pos:
